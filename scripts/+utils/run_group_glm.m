...
for ii = 1:nstates
    for jj = 1:nsubjs
        [copes(ii,:,:,jj),varcopes(ii,:,:,jj),tstats(ii,:,:,jj)] = wh.run_glm( squeeze(data(ii,:,:,jj))',...
    ...
    if use_flame==1
        ...
        for jj = 1:ncontrasts
            [group_copes(ii,jj,:),~,group_tstats(ii,jj,:)] = wh.run_flame( squeeze(copes(ii,jj,:,:))',...
                                                                       squeeze(varcopes(ii,jj,:,:))',...
                                                group_level_design_matrix,group_level_contrasts);
        end
    else
        for jj = 1:ncontrasts
            [group_copes(ii,jj,:),~,group_tstats(ii,jj,:)] = wh.run_glm( squeeze(copes(ii,jj,:,:))',...
                                                group_level_design_matrix,group_level_contrasts);
        end        
    end
end
    ...
    for ii = 2:nperms
        ...
        for jj = 1:nstates% per state
            ...
            if use_flame==1
                % Group level estimate
                for kk = 1:ncontrasts
                    [perm_copes(jj,kk,:),~,~] = wh.run_flame( squeeze(copes(jj,kk,:,:))',...
                        squeeze(varcopes(jj,kk,:,:))',...
                        group_level_design_matrix.*perms,group_level_contrasts);
                end
            else
                for kk = 1:ncontrasts
                    [perm_copes(jj,kk,:),~,~] = wh.run_glm( squeeze(copes(jj,kk,:,:))',...
                        group_level_design_matrix.*perms,group_level_contrasts);
                end
            end
